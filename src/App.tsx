...
export default function App() {
  ...
  const [profile, setProfile] = useState({ name: '', phone: '', email: '', address: '', postalCode: '' });
  ...
  const refreshServerState = async () => {
    try {
      ...
      console.error('Failed to load server database states:', err);
    }
  };
  ...
  useEffect(() => {
    if (currentUser) {
      setProfile({
        ...
        phone: currentUser.phone || '09399311875',
        address: currentUser.address || 'مرکزی، محلات، میدان مصطفی خمینی، گالری اونتورین',
        postalCode: currentUser.postalCode || '1435678901'
      });
    } else {
      setProfile({
        name: '',
  ...
  const handleUpdateProfile = (p: any) => {
    setProfile(p);
    // Sync to backend user details if desired
    alert('تغییرات پروفایل با موفقیت در این جلسه اعمال شد.');
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-between font-sans selection:bg-gold-200" id="app-root">
      ...
      <div
        id="cart-toast"
        className="fixed bottom-24 left-6 z-50 flex items-center gap-2.5 rounded-2xl bg-stone-900 border border-gold-800/40 text-stone-50 px-5 py-3.5 shadow-xl transition-all duration-300 transform translate-y-4 opacity-0 pointer-events-none"
      >
        <span className="flex h-5 w-5 items-center justify-center rounded-full bg-gold-500 text-stone-950 font-bold">✓</span>
        <span className="text-xs font-semibold">تغییرات با موفقیت ثبت شد.</span>
      </div>
        ...
        {isAuthOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              {/* Helper login details for easy validation */}
              <div className="mt-6 border-t border-slate-800 pt-4 bg-slate-950/40 rounded-2xl p-4 border border-slate-850">
                <span className="text-[9px] font-bold text-slate-500 block mb-2">اطلاعات ورود آسان جهت آزمایش (کلیک کنید):</span>
                <div className="flex flex-col gap-2">
                  <button
                    onClick={() => {
                      setAuthEmail('admin@aventurin.com');
                      setAuthPassword('admin123');
                      setAuthTab('login');
                    }}
                    type="button"
                    className="w-full text-right hover:bg-slate-850 px-2 py-1.5 rounded-lg text-[10px] text-gold-400/80 transition-all flex items-center justify-between"
                  >
                    <span>🔑 اکانت مدیر سیستم: admin@aventurin.com</span>
                  ...
                  <button
                    ...
                    type="button"
                    className="w-full text-right hover:bg-slate-850 px-2 py-1.5 rounded-lg text-[10px] text-slate-400 transition-all flex items-center justify-between"
                  >
                    <span>🔑 اکانت کاربر عادی: maryam@example.com</span>
